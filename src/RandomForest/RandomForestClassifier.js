...
export class RandomForestClassifier extends RandomForestBase {
   ...
   * @param {number} [options.maxFeatures=1.0] - the number of features used
   * on each estimator.
   *        * if is an integer it selects maxFeatures elements over the
   * sample features.
   *        * if is a float between (0, 1), it takes the percentage of features.
   * @param {boolean} [options.replacement=true] - use replacement over the
   * sample features.
   * @param {number} [options.seed=42] - seed for feature and samples
   * selection, must be a 32-bit integer.
   * @param {number} [options.nEstimators=50] - number of estimator to use.
   * @param {object} [options.treeOptions={}] - options for the tree
   * classifier, see [ml-cart]{@link https://mljs.github.io/decision-tree-cart/}
   * @param {string} [options.selectionMethod="mean"] - the way to calculate
   * the prediction from estimators, "mean", "median" and "mode" are supported.
   * @param {boolean} [options.useSampleBagging=false] - use bagging over
   * training samples.
