...
export class RandomForestBase {
   ...
   * @param {number} [options.maxSamples] - the number of samples used on
   * each estimator, only used when useSampleBagging=0
   *        * if is an integer it selects maxFeatures elements over the
   * sample features.
   *        * if is a float between (0, 1), it takes the percentage of features.
   * @param {number|String} [options.maxFeatures] - the number of features
   * used on each estimator.
   *        * if is an integer it selects maxFeatures elements over the
   * sample features.
   *        * if is a float between (0, 1), it takes the percentage of features.
   * @param {boolean} [options.replacement] - use replacement over the sample
   * features.
   * @param {number} [options.seed] - seed for feature and samples selection,
   * must be a 32-bit integer.
   * @param {number} [options.nEstimators] - number of estimator to use.
   * @param {object} [options.treeOptions] - options for the tree classifier,
   * see [ml-cart]{@link https://mljs.github.io/decision-tree-cart/}
   * @param {boolean} [options.isClassifier] - boolean to check if is a
   * classifier or regression model (used by subclasses).
   * @param {boolean} [options.useSampleBagging] - use bagging over training
   * samples.
  ...
  /**
   * Method that returns the way the algorithm generates the predictions, for
   * example, in classification
   * you can return the mode of all predictions retrieved by the trees, or in
   * case of regression you can
