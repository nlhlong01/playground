...
export function makeGUI() {
  ...
  const x = d3.scale.linear().domain([-1, 1]).range([0, 144]);
...
function drawNetwork(network: nn.Node[][]): void {
  ...
  const calloutThumb = d3
    .select(".callout.thumbnail")
    .style("display", "none");
  const calloutWeights = d3
  ...
  for (let layerIdx = 1; layerIdx < numLayers - 1; layerIdx++) {
    ...
    for (let i = 0; i < numNodes; i++) {
      ...
      for (let j = 0; j < node.inputLinks.length; j++) {
        const link = node.inputLinks[j];
        const path: SVGPathElement = drawLink(link,
          node2coord,
          network,
          container,
          j === 0,
        ...
        if (targetIdWithCallout == null
          ...
          const midPoint = (
            path.getPointAtLength(path.getTotalLength() * 0.7)
          );
...
function updateDecisionBoundary(network: nn.Node[][], firstTime: boolean) {
  ...
  const xScale = d3.scale.linear().domain([0, DENSITY - 1]).range(xDomain);
  const yScale = d3.scale.linear().domain([DENSITY - 1, 0]).range(xDomain);
